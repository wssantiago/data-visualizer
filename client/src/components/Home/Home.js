...
function Home() {
  ...
  return (
    <MainContainer>
      <NavForm users={people.users} submitFormHandler={submit} />
      <TitleContainer>
        <StyledTitle>
          {" "}
          {/* when JSON users data is empty, the inner HTML changes}*/}
          {people.users.length
            ? "Visualise your data!"
      ...
      <DataContainer>
        <MUITableContainer>
          ...
          {/* the Table component only renders if there are users to display*/}
        </MUITableContainer>
        <MUIChartContainer>
          {people.users.length && <Donut users={people.users}></Donut>}{" "}
          {/* the Donut component only renders if there are users to display*/}
        </MUIChartContainer>
      </DataContainer>
    </MainContainer>
  );
